...
layer(WelcomeEmailTarget.layer, { excludeTestServices: true })(
  "Cloud welcome email",
  (it) => {
    it.effect(
      ...
      () =>
        Effect.gen(function* () {
          ...
          expect(
            yield* target.messages(email, "welcome to executor"),
          ).toHaveLength(0);
          ...
          expect(
            welcome?.text.split(
              "\n\nand if you'd rather not get these at all",
            )[0],
          ).toBe(`hey Taylor,
...
if you get stuck or have questions, just reply. this was an automated email but replies go straight to me, and i'd love to hear what you're using executor for.`);
          ...
          expect(
            (yield* target.preference(
              "POST",
              token.slice(0, -10) + "AAAAAAAAAA",
            )).status,
          ).toBe(400);
          expect((yield* target.preference("POST", "not-a-token")).status).toBe(
            400,
          );
          expect((yield* target.preference("POST", token)).status).toBe(200);
          expect(
            (yield* target.preference("POST", token, "multipart")).status,
          ).toBe(200);
          const confirmed = yield* target.preference("POST", token, "browser");
          expect(confirmed.status).toBe(303);
          expect(confirmed.location).toBe(
            `${target.origin}/email/unsubscribe?result=unsubscribed`,
          );
