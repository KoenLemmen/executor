...
const agentWorkflows = [
  {
    ...
    logo: claudeLogo,
    use: "Build it",
    prompt: "Add PostHog. Only allow reads.",
    tool: "Executor · deployApp",
    result: "PostHog app deployed.",
  },
  {
    name: "Codex",
    logo: codexLogo,
    use: "Analyze with it",
    prompt: "How are signups doing?",
    tool: "PostHog · Query",
    result: "Here’s this week’s signup trend.",
  },
  {
    name: "Cursor",
    logo: cursorLogo,
    use: "Code with it",
    prompt: "Check the signup errors.",
    tool: "PostHog · Query",
    result: "Found the errors behind this bug.",
  },
];

/** One hosted app used by three agents; examples illustrate shared access, not live activity. */
export function AgentReuse() {
  return (
    <div className="agent-reuse mt-6 p-[24px] [border:1px_solid_rgb(0_0_0_/_12%)] rounded-[12px] bg-[#fff] max-[639px]:p-[16px]">
      <div className="agent-reuse-cloud flex items-center gap-6 py-[18px] px-[20px] [border:1px_solid_rgb(0_0_0_/_12%)] rounded-[8px] bg-[#fafafa] max-[639px]:flex-col max-[639px]:items-start max-[639px]:gap-4 max-[639px]:p-[16px]">
        <div className="agent-reuse-brand flex items-center gap-2.5 [&_img]:shrink-0 [&_strong]:text-[14px] [&_strong]:font-medium [&_small]:block [&_small]:mt-0.75 [&_small]:text-[10px] [&_small]:text-[#888]">
          <img src="/favicon-192.png" alt="" width="24" height="24" />
          <div>
            <strong>Executor</strong>
            <small>Your apps live here.</small>
          </div>
        </div>
        <div className="agent-reuse-app flex items-center flex-wrap gap-3 ml-auto text-[11px] text-[#888] [&_strong]:text-[12px] [&_strong]:font-medium [&_strong]:text-[#444] [&_>_span]:pl-3 [&_>_span]:[border-left:1px_solid_rgb(0_0_0_/_12%)] max-[639px]:ml-0 max-[639px]:gap-2 max-[639px]:[&_>_span]:pl-2">
          <strong>PostHog</strong>
      ...
      <div
        className="agent-reuse-branches grid grid-cols-[repeat(3,_1fr)] gap-3.5 h-8 [&_i]:w-0.25 [&_i]:h-full [&_i]:[justify-self:center] [&_i]:bg-[#ddd] max-[639px]:hidden"
        aria-hidden="true"
      >
        <i />
        <i />
      ...
      <div className="agent-reuse-clients grid grid-cols-3 gap-3.5 max-[639px]:grid-cols-[1fr] max-[639px]:mt-4.5">
        {agentWorkflows.map((agent) => (
          <div
            className="agent-reuse-client min-w-0 py-[18px] px-[14px] [border:1px_solid_rgb(0_0_0_/_10%)] rounded-[8px] max-[639px]:py-[20px] max-[639px]:px-[16px]"
            key={agent.name}
          >
            <div className="agent-reuse-client-header flex items-start gap-2.25 [&_img]:shrink-0 [&_strong]:text-[13px] [&_strong]:font-medium [&_small]:block [&_small]:mt-1 [&_small]:text-[#999] [&_small]:text-[10px] max-[639px]:[&_strong]:text-[16px] max-[639px]:[&_small]:text-[12px]">
              <img src={agent.logo} alt="" width="26" height="26" />
              <div>
                <strong>{agent.name}</strong>
                <small>{agent.use}</small>
              </div>
            </div>
            <div className="agent-reuse-question grid gap-1.5 mt-6 [&_>_span]:text-right [&_>_span]:text-[#999] [&_>_span]:text-[10px] [&_p]:m-0 [&_p]:min-h-15.5 [&_p]:p-[10px] [&_p]:rounded-[8px_8px_2px_8px] [&_p]:text-[12px] [&_p]:leading-[1.6] [&_p]:bg-[#f1efeb] max-[639px]:[&_p]:text-[16px] max-[639px]:[&_p]:min-h-0">
              <span>You</span>
              <p>{agent.prompt}</p>
            </div>
            <div className="agent-reuse-call flex items-center gap-1.5 py-[12px] px-0 mt-3.5 [font:9px_var(--font-mono)] text-[#888] [border-top:1px_solid_rgb(0_0_0_/_7%)] [border-bottom:1px_solid_rgb(0_0_0_/_7%)] [&_img]:shrink-0 max-[639px]:text-[11px]">
              <img src="/favicon-192.png" alt="" width="14" height="14" />
              <span>{agent.tool}</span>
              <span className="viz-check text-[#287359] [.agent-reuse-call_&]:ml-auto">✓</span>
            </div>
            <p className="agent-reuse-result [margin:14px_0_0] text-[#777] text-[12px] leading-[1.6] max-[639px]:text-[16px]">
              {agent.result}
            </p>
          </div>
        ))}
      </div>
      <div className="agent-reuse-footer flex justify-between flex-wrap gap-2.5 pt-5.5 text-[#888] text-[11px] [&_strong]:font-medium [&_strong]:text-[#555] max-[639px]:text-[12px] max-[639px]:leading-[1.5]">
        <strong>Switch agents. Keep your setup.</strong>
        <span>Same app. Same account. Same rules.</span>
      </div>
    </div>
  );
}
