      ...
      Effect.scoped(
        Effect.gen(function* () {
          ...
          yield* Effect.gen(function* () {
            ...
            assert.ok(
              Schema.is(AccountRequired)(yield* Tools.listTools({ app: app.id }).pipe(Effect.flip)),
            );
            assert.ok(
            ...
            assert.ok(
              Schema.is(AppNotFound)(yield* beta.listTools({ app: app.id }).pipe(Effect.flip)),
            );
            assert.ok(Schema.is(AppNotFound)(yield* beta.callTool(input).pipe(Effect.flip)));
            assert.deepEqual(
              (yield* member.listTools({ app: app.id })).items.map((tool) => tool.name),
              ["mutations.greeting"],
            );
            assert.ok(
