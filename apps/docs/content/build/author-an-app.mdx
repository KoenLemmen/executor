...
package. The host supplies that package. One `index.ts` is enough; you only need
a `package.json` if you add a third-party dependency.
...
There is no `executor deploy` command. The Executor CLI starts the local server;
it does not deploy app source.

## What is coming later

- Updating hosted app source through the deploy call. It currently creates a new
  named app and rejects a name that already exists. Local deploy does update in
  place.
