// Adapted from the old repo's apps/cli/src/release.ts. Path resolution only
// has been changed to fit this repo's layout; channel and tagging logic is
// unchanged. The `apps/cli` package this script targets does not exist yet
// (tracked by #229) and no workflow calls this script yet (tracked by #231).
...
const syncGitHubRelease = async (input: {
  ...
  // Draft until a desktop-publish workflow finishes uploading installers and
  // flips it; otherwise /releases/latest/download/<desktop-asset> 404s during
  // the desktop build window.
  const args = [
    "release",
    "create",
    input.tag,
...
const locateBuiltArtifacts = async (version: string): Promise<string> => {
  ...
  // platform variants this release ships. The build step globs the
