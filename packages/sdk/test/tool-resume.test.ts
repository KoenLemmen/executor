    ...
    Effect.runPromise(
      Effect.scoped(
        Effect.gen(function* () {
          ...
          const rows = yield* f.options.storage.orm("1.8.2").findMany("toolApprovals", {});
    ...
    Effect.runPromise(
      Effect.scoped(
        Effect.gen(function* () {
          const f = yield* fixture;
          const request = yield* pending(f);
          const db = f.options.storage.orm("1.8.2");
          assert.ok(
    ...
    Effect.runPromise(
      Effect.scoped(
        Effect.gen(function* () {
          ...
          assert.deepEqual(
            (yield* f.options.storage.orm("1.8.2").findMany("toolApprovals", {})).map(
              ({ id }) => id,
            ),
            [live.requestId],
          );
          yield* atTime(live.expiresAt, f.executor.tools.pruneApprovals());
          assert.deepEqual(yield* f.options.storage.orm("1.8.2").findMany("toolApprovals", {}), []);
        }).pipe(Effect.provide(services)),
      ),
    ),
);
  ...
  Effect.runPromise(
    Effect.scoped(
      Effect.gen(function* () {
        ...
        assert.deepEqual(
          (yield* f.options.storage.orm("1.8.2").findMany("toolApprovals", {})).map(({ id }) => id),
          [fresh.requestId],
        );
      }).pipe(Effect.provide(services)),
    ),
